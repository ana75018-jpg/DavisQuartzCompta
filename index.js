...
// Heure d'été/hiver européenne (règle légale : bascule le dernier dimanche de mars/octobre à 01:00 UTC)
// — utilisé uniquement pour la conversion des horodatages bancaires (xbankaccount) pour l'instant.
function isEuropeSummerTime(date) {
  ...
  const marchLastSun = lastSunday(2); marchLastSun.setUTCHours(1, 0, 0, 0);
  const octLastSun = lastSunday(9); octLastSun.setUTCHours(1, 0, 0, 0);
...
async function parseExport(msg) {
  for (const embed of (msg.embeds || [])) {
    const titleRaw = (embed.title || embed.author?.name || '');
    const title = titleRaw.toLowerCase()
      .replace(/[\u{1F000}-\u{1FFFF}\u{2600}-\u{27FF}\u{2700}-\u{27BF}]/gu, '')
      .replace(/[*_~]/g, '').trim();
    if (!title.includes('export') && !title.includes('xaction') && !title.includes('carton')) continue;

    // Parser description d'abord, fields par-dessus (fields = prioritaires)
    let data = parseDesc(embed.description || '');
    if (embed.fields && embed.fields.length > 0) {
      Object.assign(data, parseFields(embed.fields));
    }

    console.log(`[EXPORT-DEBUG] Titre: "${title}" | data keys: ${Object.keys(data).join(', ')}`);

    // Nom RP : priorité à propername (format xaction), sinon nom/employe
    const nomRaw = data['propername'] || data['nom'] || data['employe'] || data['employee'] || data['identite'] || data['name'] || '';
    ...
    // Character ID / Source
    const idPerso = data['characterid'] || data['idperso'] || data['source'] || '';

    const ts = msg.createdAt;
    if (!nom) { console.log('[EXPORT] Nom vide, ignoré. data:', JSON.stringify(data).slice(0,200)); continue; }
...
async function parseIG(msg) {
  for (const embed of (msg.embeds || [])) {
    ...
    // ── Export de Cartons + xaction-shop ───────────────
    if (title.includes('export') || title.includes('xaction')) {
      const data = {};
      (embed.fields || []).forEach(f => {
        const key = (f.name || '').toLowerCase()
          .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
          .replace(/[^a-z0-9]/g, '');
        const val = (f.value || '').trim();
        // Format "properName:Issa Blackys" → extraire après le premier :
        const m = val.match(/^[^:]+:(.+)$/);
        data[key] = m ? m[1].trim() : val;
      });
      ...
      // Nom RP : propername (xaction) ou nom/employe (ancien format)
      const nomRaw = data['propername'] || data['nom'] || data['name'] || data['employe'] || '';
      const nom = extractNom(nomRaw);

      // Quantité : amount (xaction) ou quantiteexportee (ancien)
      const qStr = data['amount'] || data['quantiteexportee'] || data['quantiteexporte'] || data['quantite'] || '0';
      const quantite = parseInt(qStr.replace(/[^\d]/g, '')) || 0;
      ...
      if (!nom) { console.log('[EXPORT-IG] Nom vide, ignoré. data:', JSON.stringify(data).slice(0,200)); continue; }

      const row = { id: msg.id, nom, id_perso: idPerso, quantite, total_semaine: totalSem, timestamp: ts.toISOString(), semaine: getWeekNum(ts), message_id: msg.id };
      const { error } = await sb.from('discord_exports').upsert(row, { onConflict: 'id' });
      if (error) console.error('[EXPORT-IG] ERR:', error.message);
      else console.log(`[EXPORT-IG] 📦 ${nom} — ${quantite} (total sem: ${totalSem})`);
      continue;
    }

    // ── Comptes bancaires (xbankaccount) : retraits / dépôts / paiements sur le compte DAVIS ──
    if (title.includes('xbankaccount')) {
      const data = {};
      (embed.fields || []).forEach(f => {
        const key = (f.name || '').toLowerCase()
          .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
      ...
      // ── Paiement (xbankaccount – paid) : alimente l'onglet Facture (CA) ──
      // Pas de champ iban ici. fromPropername = l'employé DQ qui a vendu/encaissé (le Vendeur).
      if (title.includes('paid')) {
        const amount = parseFloat((data['amount'] || '0').replace(/[^\d.,-]/g, '').replace(',', '.')) || 0;
        const reason = data['reason'] || '';
        const vendeur = data['frompropername'] || '';
        const discordId = data['fromdiscord'] || '';
        ...
        if (dateMatch) {
          const [, day, month, year, h, min, sec] = dateMatch;
          const provisoire = new Date(Date.UTC(+year, +month - 1, +day, +h, +min, +sec));
          const decalage = isEuropeSummerTime(provisoire) ? 2 : 1;
          ts = new Date(Date.UTC(+year, +month - 1, +day, +h - decalage, +min, +sec));
        }
        ...
        const { error } = billId
          ? await sb.from('discord_bank_payments').upsert(row, { onConflict: 'bill_id' })
          : await sb.from('discord_bank_payments').insert(row);
        if (error) console.error('[BANK] discord_bank_payments ERR:', error.message);
        else console.log(`[BANK] Facture — ${vendeur || discordId} a vendu pour ${amount}$ (${reason || 'sans motif'})`);
        continue;
      }

      const COMPANY_IBAN = 'DAVIS';
      const iban = data['iban'] || '';
      if (iban.toUpperCase() !== COMPANY_IBAN) { continue; } // pas le compte DQ, on ignore

      const table = title.includes('withdraw') ? 'discord_bank_withdraw'
                  : title.includes('addmoney') ? 'discord_bank_addmoney'
                  : null;
      if (!table) { continue; }

      const amount = parseFloat((data['amount'] || '0').replace(/[^\d.,-]/g, '').replace(',', '.')) || 0;
      const beforeRaw = (data['before'] || '').replace(/[^\d.,-]/g, '').replace(',', '.');
      const afterRaw  = (data['after']  || '').replace(/[^\d.,-]/g, '').replace(',', '.');
      const before = beforeRaw === '' ? NaN : parseFloat(beforeRaw);
      const after  = afterRaw  === '' ? NaN : parseFloat(afterRaw);
      const reason = data['reason'] || '';
      const properName = data['propername'] || '';
      const discordId = data['discord'] || '';

      // L'heure du jeu est en heure de Paris, pas en UTC — le serveur Railway tourne en UTC.
      // On convertit explicitement (même correctif que sur LTD, pour éviter le décalage +1h/+2h).
      let ts = msg.createdAt;
      const dateRaw = data['date'] || '';
      const dateMatch = dateRaw.match(/(\d{2})\/(\d{2})\/(\d{4})\s+(\d{2}):(\d{2}):(\d{2})/);
      if (dateMatch) {
      ...
      if (error) console.error(`[BANK] ${table} ERR:`, error.message);
      else console.log(`[BANK] ${table} — ${properName || discordId} ${amount}$ (${reason || 'sans motif'}) — after: ${isNaN(after) ? '?' : after}`);
      continue;
    }
  ...
  try {
    ...
    while (true) {
      ...
      for (const msgData of msgs) {
        ...
        // Debug : logger le message cible
        if (msgData.id === '1515418786180890707') {
          console.log('[DEBUG-TARGET] TROUVE! embeds:', msgData.embeds?.length, '| content:', JSON.stringify(msgData.content).slice(0,100), '| webhook_id:', msgData.webhook_id, '| type:', msgData.type);
          console.log('[DEBUG-TARGET] embeds raw:', JSON.stringify(msgData.embeds).slice(0,500));
        }
...
// ═══════════════════════════════════════════════════
// SYNC AUTO QUANTITÉ (coffre → app_data)
// Port fidèle de syncQuantiteFromCoffre() du portail web, mais exécuté ici côté
// serveur (Railway) toutes les 5 min — donc indépendant de tout onglet Direction ouvert.
// ═══════════════════════════════════════════════════

// Uniquement le groupe Contenaire (50628), seul groupe utilisé par coffreCategorieMinerai par défaut.
const COFFRE_NAMES_50628 = {
  'action-50628-0-1': '🟠 Cuivre',
  'action-50628-0-2': '⚫ Charbon',
  'action-50628-0-3': '🔵 Fer',
  'action-50628-0-4': '🟠 Cuivre 1',
  'action-50628-0-5': '⚪ Aluminium',
  'action-50628-0-6': '🔵 Fer 2',
  'action-50628-0-7': '👑 Direction',
  'action-50628-0-8': '🟡 Or',
  'action-50628-0-9': '📦 Cartons',
};

function getCoffreNameServer(owner) {
  if (!owner) return '—';
  if (COFFRE_NAMES_50628[owner]) return COFFRE_NAMES_50628[owner];
  for (const [key, name] of Object.entries(COFFRE_NAMES_50628)) {
    if (owner === key || owner.endsWith(key) || owner === 'owner:' + key) return name;
  }
  return owner;
}

// Identique à coffreCategorieMinerai() du portail web : seuls les coffres du groupe
// Contenaire (50628) comptent, jamais Mine (39805) ni Pioches (85135).
function coffreCategorieMineraiServer(owner, groupId) {
  groupId = groupId || '50628';
  if (!owner || !owner.includes(groupId)) return null;
  const name = getCoffreNameServer(owner);
  if (!name || name === '—') return null;
  const n = name.toLowerCase().replace(/[^a-z0-9\/\s]/g, '').trim();
  if (n.includes('direction')) return null;
  if (n.includes('carton')) return 'CartonsOr';
  if (n.includes('ferr')) return null;
  if (n.includes('stock')) return null;
  if (n.includes('/')) return null;
  if (/^ch\s*\d*$/.test(n) || n.includes('charbon')) return 'Charbon';
  if (/^fer\s*\d*$/.test(n)) return 'Fer';
  if (/^cu\s*\d*$/.test(n) || n.includes('cuivre')) return 'Cuivre';
  if (/^alu\s*\d*$/.test(n) || n.includes('aluminium')) return 'Aluminium';
  if (/^m?or\s*\d*$/.test(n)) return 'Or';
  return null;
}

// Bornes lundi 00h00 → dimanche 23h59 en heure de Paris (même semaine que verrait
// un navigateur ouvert en France), converties en UTC réel pour la requête Supabase.
function getParisWeekBounds(now = new Date()) {
  const decalage = isEuropeSummerTime(now) ? 2 : 1;
  const parisNow = new Date(now.getTime() + decalage * 3600000);
  const day = parisNow.getUTCDay() || 7;
  const lundiParis = new Date(Date.UTC(parisNow.getUTCFullYear(), parisNow.getUTCMonth(), parisNow.getUTCDate() - (day - 1), 0, 0, 0));
  const dimancheParis = new Date(lundiParis.getTime() + 7 * 86400000 - 1);
  return {
    start: new Date(lundiParis.getTime() - decalage * 3600000),
    end: new Date(dimancheParis.getTime() - decalage * 3600000),
  };
}

// Parse une date française "dd/mm/yyyy" (celle stockée sur chaque semaine) en Date UTC midi,
// pour éviter tout souci de bord de journée.
function parseFrDateServer(str) {
  const m = (str || '').match(/(\d{2})\/(\d{2})\/(\d{4})/);
  if (!m) return new Date();
  return new Date(Date.UTC(+m[3], +m[2] - 1, +m[1], 12));
}

async function syncQuantiteFromCoffreServer() {
  try {
    // Charger l'état courant partagé avec le portail D'ABORD, pour connaître la date de
    // création de la semaine active — la fenêtre lundi-dimanche doit être calculée à PARTIR
    // de cette date, jamais à partir d'aujourd'hui. Sinon dès que le vrai calendrier passe un
    // lundi (avant même que Direction ait cliqué sur "+ Nouvelle semaine"), on calculerait la
    // fenêtre de la nouvelle semaine (quasi vide) et on l'écraserait sur l'ancienne, toujours active.
    const { data: appData, error: err2 } = await sb.from('app_data').select('key,value').in('key', ['dq_weeks', 'dq_qty_overrides']);
    if (err2) throw err2;
    const map = {};
    (appData || []).forEach(r => { map[r.key] = r.value; });
    const weeks = map.dq_weeks;
    const qtyOverrides = new Set((Array.isArray(map.dq_qty_overrides) ? map.dq_qty_overrides : []).map(String));
    if (!Array.isArray(weeks) || !weeks.length) {
      console.log('[COFFRE-SYNC-SRV] dq_weeks introuvable ou vide, abandon');
      return;
    }
    // Toujours la dernière semaine créée (semaine active) — jamais dq_curidx, qui suit
    // simplement la semaine que Direction est en train de consulter dans son navigateur.
    const weekIdx = weeks.length - 1;
    if (!weeks[weekIdx] || !Array.isArray(weeks[weekIdx].data)) {
      console.log('[COFFRE-SYNC-SRV] Semaine active vide, abandon');
      return;
    }
    const cwData = weeks[weekIdx].data;
    const refDate = parseFrDateServer(weeks[weekIdx].date);
    const { start, end } = getParisWeekBounds(refDate);

    // Charger tous les mouvements add/remove de la semaine (pagination 1000, comme le portail)
    let rows = [], from = 0;
    while (true) {
      const { data, error } = await sb.from('discord_inventaire')
        .select('*')
        .gte('timestamp', start.toISOString())
        .lte('timestamp', end.toISOString())
        .in('action', ['add', 'remove'])
        .range(from, from + 999);
    ...
    cwData.forEach(row => {
      const pole = row.categorie;
      if (!['Charbon', 'Fer', 'Cuivre', 'Or', 'Aluminium'].includes(pole)) return;
      if (row._decl) return; // ne jamais toucher une vieille ligne de déclaration
      if (qtyOverrides.has(String(row.casier))) { skipped++; return; } // corrigé à la main, protégé
      const nomLower = (row.nom || '').toLowerCase().trim();
      if (!nomLower) return;

      const mesMouvements = rows.filter(r => (r.proper_name || r.nom || '').toLowerCase().trim() === nomLower);
      const mouvementsRetenus = mesMouvements.filter(r => r.action === 'add'); // dépôts uniquement

      const totalMinerai = mouvementsRetenus
        .filter(r => coffreCategorieMineraiServer(r.owner) === pole)
        .reduce((s, r) => s + (parseInt(r.count) || 0), 0);

      if ((row.quantite || 0) !== totalMinerai) count++;
      row.quantite = totalMinerai; // toujours écrasé, même à 0 — plus de valeur périmée

      if (pole === 'Or') {
        const totalCartons = mouvementsRetenus
          .filter(r => coffreCategorieMineraiServer(r.owner) === 'CartonsOr')
          .reduce((s, r) => s + (parseInt(r.count) || 0), 0);
        if ((row.cartonsOr || 0) !== totalCartons) count++;
        row.cartonsOr = totalCartons;
      }
    });

    if (count > 0) {
      const { error: err3 } = await sb.from('app_data').upsert({ key: 'dq_weeks', value: weeks }, { onConflict: 'key' });
      if (err3) throw err3;
      console.log(`[COFFRE-SYNC-SRV] ✓ ${count} champ(s) mis à jour, ${skipped} casier(s) protégé(s) ignoré(s) (semaine ${weekIdx})`);
    } else {
      console.log('[COFFRE-SYNC-SRV] Rien à mettre à jour');
    }
  } catch (e) {
    console.error('[COFFRE-SYNC-SRV] Erreur:', e.message);
  }
}
...
client.on(Events.ClientReady, async () => {
  ...
  // CH_EXPORT désactivé — exports gérés uniquement via CH_IG
  await fetchHistoryREST(CHANNELS.IG, parseIG, 'IG', 2000);
  // Pas de fetch historique pour FRAIS — écoute temps réel uniquement
  console.log('✅ Historique chargé — écoute temps réel active');

  // Sync coffre → Quantité, côté serveur, indépendante de tout onglet Direction ouvert
  await syncQuantiteFromCoffreServer();
  setInterval(syncQuantiteFromCoffreServer, 5 * 60 * 1000);

  // Re-fetch automatique toutes les 15 minutes
  setInterval(async () => {
    console.log('[AUTO] Re-fetch périodique...');
    // CH_EXPORT désactivé — exports gérés uniquement via CH_IG
    // Pas de re-fetch FRAIS
  }, 15 * 60 * 1000);
});
