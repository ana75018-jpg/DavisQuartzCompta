...
async function syncQuantiteFromCoffreServer() {
  try {
    ...
    const { data: appData, error: err2 } = await sb.from('app_data').select('key,value').in('key', ['dq_weeks']);
    if (err2) throw err2;
    const map = {};
    ...
    if (count > 0) {
      ...
      if (err3) throw err3;
      console.log(`[COFFRE-SYNC-SRV] ✓ ${count} champ(s) mis à jour (semaine ${weekIdx})`);
    } else {
