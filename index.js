...
async function parseExport(msg) {
  for (const embed of (msg.embeds || [])) {
    ...
    // Champs possibles : employe, identite, discord, idperso, quantiteexporte, totalsemaine, date, objectifhebdo
    const nom       = extractName(fields['employe'] || fields['identite'] || fields['nom'] || '');
    const idPerso   = fields['idperso'] || fields['identitperso'] || '';
    const quantite  = parseInt(fields['quantiteexporte'] || fields['quantite'] || '0');
    const totalSem  = parseInt(fields['totalsemaine'] || fields['total'] || '0');
    const dateStr   = fields['date'] || '';
    const ts        = msg.createdAt;

    if (!nom && !quantite) continue;
