...
async function syncQuantiteFromCoffreServer() {
  try {
    ...
    const { data: appData, error: err2 } = await sb.from('app_data').select('key,value').in('key', ['dq_weeks', 'dq_curidx']);
    if (err2) throw err2;
    const map = {};
    (appData || []).forEach(r => { map[r.key] = r.value; });
    const weeks = map.dq_weeks;
    const curIdx = map.dq_curidx ?? 0;
    if (!Array.isArray(weeks) || !weeks[curIdx] || !Array.isArray(weeks[curIdx].data)) {
      console.log('[COFFRE-SYNC-SRV] dq_weeks introuvable ou vide, abandon');
      return;
    }
    const cwData = weeks[curIdx].data;
    ...
    if (count > 0) {
      ...
      if (err3) throw err3;
      console.log(`[COFFRE-SYNC-SRV] ✓ ${count} champ(s) mis à jour (semaine ${curIdx})`);
    } else {
