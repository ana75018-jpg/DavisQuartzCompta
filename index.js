...
function parseFields(fields) {
  ...
  (fields || []).forEach(f => {
    const key = (f.name || '').toLowerCase()
      ...
      .replace(/[→•\s]/g, '')
      .replace(/[^a-z0-9]/g, '');
    // Nettoyer la valeur : retirer backticks, markdown bold, mentions
    const val = (f.value || '').replace(/`/g, '').replace(/\*\*/g, '').trim();
    data[key] = val;
  });
  return data;
}
...
async function parseFrais(msg) {
  ...
  for (const embed of allSources) {
    ...
    const nomRaw = data['identite'] || data['employe'] || data['employe'] || data['nom'] || data['employee'] || data['employrp'] || '';
    const nom = extractNom(nomRaw);
    const montantRaw = data['montant'] || data['amount'] || data['montant$'] || data['montants'] || '0';
    const montant = parseMontant(montantRaw);
    const raison = data['raison'] || data['motif'] || data['description'] || '';
    console.log(`[FRAIS-DEBUG] nomRaw="${nomRaw}" montantRaw="${montantRaw}" keys=${Object.keys(data).join(',')}`);
...
client.on(Events.MessageCreate, async (msg) => {
  ...
  // Channel frais : utiliser REST API pour avoir les embeds des bots Application
  if (isFraisChannel(id)) {
    const tryFraisREST = async (delayMs) => {
      await sleep(delayMs);
      try {
        // Fetch les derniers messages du channel pour trouver celui avec l'embed
        const res = await fetch(`https://discord.com/api/v10/channels/${id}/messages?limit=10`, {
          headers: { Authorization: `Bot ${DISCORD_TOKEN}` }
        });
        if (!res.ok) { console.error(`[FRAIS-REST] HTTP ${res.status}`); return false; }
        const msgs = await res.json();
        // Chercher le message avec embed autour du même timestamp (±30s)
        const refTs = msg.createdTimestamp || Date.now();
        const candidate = msgs.find(m => m.embeds?.length > 0 && Math.abs(new Date(m.timestamp).getTime() - refTs) < 30000);
        console.log(`[FRAIS-REST] delay=${delayMs}ms | ${msgs.length} msgs | candidat: ${candidate?.id||'aucun'} | embeds: ${candidate?.embeds?.length||0}`);
        if (!candidate) return false;
        const fakeMsg = {
          id: candidate.id, channelId: id,
          content: candidate.content || '',
          embeds: candidate.embeds || [],
          components: candidate.components || [],
          reactions: { cache: new Map() },
          createdAt: new Date(candidate.timestamp),
          author: { bot: candidate.author?.bot, tag: candidate.author?.username },
          fetch: async () => fakeMsg
        };
        await parseFrais(fakeMsg);
        return true;
      } catch(e) { console.error('[FRAIS-REST] err:', e.message); return false; }
    };
    let done = false;
    for (const delay of [2000, 6000, 15000]) {
      if (done) break;
      done = await tryFraisREST(delay);
    }
    return;
  }
...
client.on(Events.MessageReactionAdd, async (reaction, user) => {
  ...
  try {
    ...
    if (!res.ok) { console.error(`[FRAIS-REACT] HTTP ${res.status}`); return; }
    const msgData = await res.json();
    const fakeMsg = {
      id: msgData.id, channelId,
      content: msgData.content || '',
      embeds: msgData.embeds || [],
      components: msgData.components || [],
      reactions: { cache: new Map() },
...
client.on(Events.MessageUpdate, async (oldMsg, newMsg) => {
  ...
  try {
    // Utiliser REST API pour avoir les embeds des bots Application
    const res = await fetch(`https://discord.com/api/v10/channels/${channelId}/messages?limit=10`, {
      headers: { Authorization: `Bot ${DISCORD_TOKEN}` }
    });
    if (!res.ok) { console.error(`[FRAIS-UPDATE] HTTP ${res.status}`); return; }
    const msgs = await res.json();
    const candidate = msgs.find(m => m.embeds?.length > 0 && (m.id === newMsg.id || Math.abs(new Date(m.timestamp).getTime() - Date.now()) < 60000));
    console.log(`[FRAIS-UPDATE] REST | candidat: ${candidate?.id||'aucun'} | embeds: ${candidate?.embeds?.length||0} | titre: "${candidate?.embeds?.[0]?.title||''}"`);
    if (!candidate) return;
    const fakeMsg = {
