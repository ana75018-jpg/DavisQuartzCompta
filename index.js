...
function extractNom(str) {
  if (!str) return '';
  // Format ancien : "@Kayla Moreno - DM=🚫 (littlestitch_9)" → "Kayla Moreno"
  // Format ancien : "<@28839...> (itspopopo)" → vide, on ignore
  // Format nouveau : "Kayla Moreno" → "Kayla Moreno"
  let s = str;
  // Si ça commence par <@ ou @, c'est un pseudo Discord → on prend entre parenthèses si rien d'autre
  // Sinon on prend le texte avant la mention
  s = s
    .replace(/`/g, '')
    .replace(/\*\*/g, '')
    .replace(/DM=\S*/gi, '')
    .trim();
  // Extraire le texte avant une mention Discord <@...>
  const beforeMention = s.match(/^(.+?)\s*<@/);
  if (beforeMention) return beforeMention[1].replace(/@/g,'').trim();
  // Retirer les mentions Discord
  s = s.replace(/<@!?\d+>/g, '').trim();
  // Retirer les pseudos entre parenthèses si reste un nom avant
  s = s.replace(/\s*\([^)]+\)$/, '').trim();
  // Retirer les @pseudo
...
client.on(Events.MessageCreate, async (msg) => {
  ...
  // Pour les channels export et frais, traiter même sans embed
  if (id === CHANNELS.EXPORT)  { await parseExport(msg); return; }
  if (id === CHANNELS.FRAIS)   { await parseFrais(msg); return; }
