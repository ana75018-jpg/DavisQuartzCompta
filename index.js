...
async function fetchHistory(channelId, parser) {
  try {
    console.log(`[HISTORY] Fetch channel: ${channelId}`);
    const channel = await client.channels.fetch(channelId);
    if (!channel) { console.log(`[HISTORY] Channel ${channelId} introuvable`); return; }
    console.log(`[HISTORY] Channel trouvé: #${channel.name}`);
    let lastId = null;
    let total  = 0;
    while (true) {
    ...
    console.error(`[HISTORY] Erreur channel ${channelId}:`, e.message, e.code||'');
  }
}
...
client.once('clientReady', async () => {
  ...
  for(const [name, id] of Object.entries({SERVICE:CHANNELS.SERVICE, EXPORT:CHANNELS.EXPORT, IG:CHANNELS.IG, FRAIS:CHANNELS.FRAIS})){
    console.log(`[HISTORY] Tentative channel ${name}: ${id}`);
  }
