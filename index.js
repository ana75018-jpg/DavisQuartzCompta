...
async function parseIG(msg) {
  for (const embed of (msg.embeds || [])) {
    ...
    // Debug fields
    console.log(`[IG] action:${action} fields:${embed.fields?.length||0} data:${JSON.stringify(data).substring(0,150)}`);

    const row = {
      id:           data['uuid'] || (msg.id + '_' + (data['date']||'').replace(/[^0-9]/g,'')),
...
client.on(Events.MessageCreate, async (msg) => {
  ...
  if (id === CHANNELS.IG) {
    console.log(`[IG REALTIME] msg reçu - embeds:${msg.embeds.length} webhook:${msg.webhookId||'non'} author:${msg.author?.username||'?'} content:"${(msg.content||'').substring(0,30)}"`);
  }
