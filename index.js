...
client.on(Events.MessageCreate, async (msg) => {
  ...
  if (isFraisChannel(id)) {
    // Essai immédiat via REST
    const tryFraisREST = async (delayMs) => {
      await sleep(delayMs);
      try {
        const res = await fetch(`https://discord.com/api/v10/channels/${id}/messages/${msg.id}`, {
          headers: { Authorization: `Bot ${DISCORD_TOKEN}` }
        });
        if (!res.ok) { console.error(`[FRAIS-REST] HTTP ${res.status}`); return false; }
        const msgData = await res.json();
        console.log(`[FRAIS-REST] delay=${delayMs}ms | embeds: ${msgData.embeds?.length||0} | content: ${(msgData.content||'').slice(0,50)}`);
        if (!msgData.embeds?.length && !msgData.content) return false;
        const fakeMsg = {
          id: msgData.id, channelId: id,
          content: msgData.content || '',
          embeds: msgData.embeds || [],
          components: msgData.components || [],
          reactions: { cache: new Map() },
    ...
    // Tenter à 1s, 5s, 12s
    let done = false;
    for (const delay of [1000, 5000, 12000]) {
      if (done) break;
...
client.on(Events.MessageUpdate, async (oldMsg, newMsg) => {
  ...
  try {
    ...
    const res = await fetch(`https://discord.com/api/v10/channels/${channelId}/messages/${newMsg.id}`, {
      headers: { Authorization: `Bot ${DISCORD_TOKEN}` }
    });
    if (!res.ok) { console.error(`[FRAIS-UPDATE] HTTP ${res.status}`); return; }
    const msgData = await res.json();
    console.log(`[FRAIS-UPDATE] REST | embeds: ${msgData.embeds?.length||0} | titre: "${msgData.embeds?.[0]?.title||''}"`);
    if (!msgData.embeds?.length && !msgData.content) return;
    const fakeMsg = {
      id: msgData.id, channelId,
      content: msgData.content || '',
      embeds: msgData.embeds || [],
