...
async function parseFrais(msg) {
  ...
  // Si pas d'embed, essayer de lire le contenu texte du message
  ...
  for (const embed of embeds) {
    ...
    const titleRaw = embed.title || embed.description || msg.content || '';
    ...
    const desc = embed.description || '';
    // Merger fields + description + contenu texte brut
    let data = {};
    if (embed.fields && embed.fields.length > 0) {
      data = parseFields(embed.fields);
    }
    const descData = parseDesc(desc);
    Object.assign(data, descData);
    // Aussi parser le contenu texte brut si disponible
    if (msg.content) {
      const contentData = parseDesc(msg.content);
