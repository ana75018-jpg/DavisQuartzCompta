...
function parseDesc(desc) {
  ...
  (desc || '').split('\n').forEach(line => {
    ...
    const cleaned = line
      .replace(/^[\s\u00A0\u{1F000}-\u{1FFFF}\u{2600}-\u{27FF}🔷🔹📦📊📅🎯💼🆔👤🧾🟡🔵🟠⚪🏷️💵🔗📋📌🔢💳⏱️✅❌🎫🏦🧑🎮🏭💰📈🎯🔑]+/u, '')
...
async function parseFrais(msg) {
  ...
  // Sources : embeds + contenu texte brut
  ...
  // Si pas d'embed ou en complément, traiter le contenu texte comme un embed virtuel
  ...
  for (const embed of allSources) {
    ...
    // Retirer tous les emojis et caractères non-ASCII avant le test
    const title = titleRaw.toLowerCase()
      .replace(/[\u{1F000}-\u{1FFFF}\u{2600}-\u{27FF}\u{2700}-\u{27BF}✅❌🧾📋]/gu, '')
      .replace(/[*_~]/g, '').trim();
    if (!title.includes('note') && !title.includes('frais') && !title.includes('ndf') && !title.includes('nouvelle') && !title.includes('accept') && !title.includes('refus')) continue;

    let data = {};
    if (embed.fields && embed.fields.length > 0) data = parseFields(embed.fields);
    const descData = parseDesc(embed.description || '');
    Object.assign(data, descData);

    const nomRaw = data['employe'] || data['identite'] || data['nom'] || data['employee'] || '';
    const nom = extractNom(nomRaw);
    const discordId = (data['discordid'] || data['discord'] || data['id'] || '').replace(/[^\d]/g, '');
    const noteId = (data['noteid'] || data['id'] || '').replace(/[^\d]/g, '');
    ...
    if (!nom || !montant) { console.log('[FRAIS] Ignoré — nom:', nom, 'montant:', montant, 'data:', JSON.stringify(data).slice(0,200)); continue; }

    const row = { id: msg.id, nom, discord_id: discordId || undefined, note_id: noteId ? parseInt(noteId) : undefined, montant, raison, statut, payee_par: payeePar, timestamp: msg.createdAt.toISOString(), semaine: getWeekNum(msg.createdAt), message_id: msg.id };
    ...
    break; // Un seul insert par message
  }
}
...
async function fetchHistoryREST(channelId, parser, name) {
  try {
    ...
    while (true) {
      ...
      for (const msgData of msgs) {
        // Reconstruire un objet compatible avec nos parsers
        const fakeMsg = {
          id: msgData.id,
          channelId,
      ...
      await sleep(500);
      if (msgs.length < 100 || total >= 500) break;
    }
...
client.on(Events.ClientReady, async () => {
  ...
  await fetchHistoryREST(CHANNELS.FRAIS, parseFrais,  'FRAIS');
