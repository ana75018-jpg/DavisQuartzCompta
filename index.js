...
async function parseFrais(msg) {
  ...
  for (const embed of allSources) {
    ...
    if (!title.includes('note') && !title.includes('frais') && !title.includes('ndf') && !title.includes('nouvelle')) continue;
    ...
    if (!nom || !montant) { console.log('[FRAIS] Ignoré — nom:', nom, 'montant:', montant, 'data:', JSON.stringify(data).slice(0,100)); continue; }
